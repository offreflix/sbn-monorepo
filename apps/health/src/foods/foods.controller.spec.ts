...
describe('FoodsController', () => {
  ...
  describe('create', () => {
    it('should create a food', async () => {
      ...
      const dto = { name: 'Banana', servingSizeValue: 100, servingSizeUnit: 'g', caloriesPerServing: 89 };
    ...
    it('should throw when x-user-id missing', () => {
      expect(() =>
        controller.create({ name: 'Banana', servingSizeValue: 100, servingSizeUnit: 'g', caloriesPerServing: 89 }, undefined),
  ...
  describe('update', () => {
    it('should update a food', async () => {
      ...
      expect(mockFoodsService.update).toHaveBeenCalledWith('f1', 'u1', { name: 'Updated' });
    });
  });
