...
describe('MeasurementsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeasurementsService,
        { provide: MeasurementsRepository, useValue: mockMeasurementsRepository },
      ],
    }).compile();
  ...
  describe('create', () => {
    it('should create a measurement', async () => {
      ...
      const result = await service.create('u1', { weightKg: 75.5, measuredAt: '2024-03-15' });
  ...
  describe('findByDateRange', () => {
    it('should return measurements in range', async () => {
      mockMeasurementsRepository.findByDateRange.mockResolvedValue([mockMeasurement]);
      const result = await service.findByDateRange('u1', '2024-01-01', '2024-12-31');
    ...
    it('should return all measurements when no range', async () => {
      ...
      expect(mockMeasurementsRepository.findByDateRange).toHaveBeenCalledWith('u1', undefined, undefined);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException for another user measurement', async () => {
      mockMeasurementsRepository.findById.mockResolvedValue({ ...mockMeasurement, userId: 'u2' });
      await expect(service.remove('m1', 'u1')).rejects.toThrow(NotFoundException);
    });
  });
});
