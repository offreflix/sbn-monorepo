...
- **Risk**: Increased file tree depth just to render a visual list.
  *Mitigation*: Keeps separation of concerns intact. Long-term maintenance guarantees better isolated testing of the list filtering and formatting logic.
- **Risk**: Prop breaking. If we break up the single fat component, prop chaining could be disjointed.
  *Mitigation*: TypeScript strictly typed contexts (`transactions.type.ts`) provide compile errors instantly if props get disaligned.
